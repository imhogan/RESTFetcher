<xsl:stylesheet version = '2.0'
	...
	A simple XSL:FO sample for RESTFetcher processing of Survey Monkey API V3 data.
...
-->
    ...
    <xsl:param name="ApplicationTitle">RESTFetcher Survey Monkey Example</xsl:param>
    <xsl:param name="ApplicationSubTitle">Produced by THINKronicity Pty Ltd.</xsl:param>
    <xsl:param name="ReportLogoURL">../images/THINKronicityLogo.png</xsl:param>
        ...
        <fo:root>
            ...
            <fo:layout-master-set>
                <fo:simple-page-master  master-name="A4Landscape"
                    ...
                    page-width="29.7cm"
                    page-height="21cm">
                    <fo:region-body margin-top="1.6cm" margin-bottom="1.2cm"/>
                ...
                <x:xmpmeta xmlns:x="adobe:ns:meta/">
                    <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                        <rdf:Description rdf:about=""
                            ...
                            <dc:creator>Ian Hogan - Ian.Hogan@THINKronicity.com.au</dc:creator>
                            <dc:description>Sample Survey Monkey report generated by RESTFetcher.</dc:description>
                        </rdf:Description>
                        <rdf:Description rdf:about=""
                            xmlns:xmp="http://ns.adobe.com/xap/1.0/">
                            <!-- XMP properties go here -->
                            <xmp:CreatorTool>RESTFetcher by THINKronicity Pty Ltd.</xmp:CreatorTool>
                        </rdf:Description>
                    </rdf:RDF>
                </x:xmpmeta>
            </fo:declarations>      
                            ...
                            <xsl:if test="$HeaderImageURL != ''">
                                <fo:table-row>
                                    <fo:table-cell number-columns-spanned="2">
                                        <fo:block>
                                            <fo:basic-link external-destination="http://THINKronicity.com.au/" show-destination="new">
                                    ...
                                    <fo:table-row>
                                        <fo:table-cell>
                                            <fo:block xsl:use-attribute-sets="sectionHeader" padding-top="{$LineHeight}">
                                                <xsl:call-template name="EmitSurveyTitle">
                                                    <xsl:with-param name="Survey" select="."/>
                                                </xsl:call-template>
                                            </fo:block>
                                        ...
                                        <fo:table-cell>
                                            <fo:block>
                                                <xsl:call-template name="EmitProperiesTable">
                                                    <xsl:with-param name="Properties" select="*[not(contains(',title,id,',concat(',',local-name(),',')))]"/>
    ...
    <xsl:template name="EmitSurveyTitle">
        <xsl:param name="Survey"/>
        <xsl:for-each select="$Survey">
            ...
            <xsl:value-of select="concat('  Survey: ',id,' - ',title)"/>
        </xsl:for-each>
    </xsl:template>
